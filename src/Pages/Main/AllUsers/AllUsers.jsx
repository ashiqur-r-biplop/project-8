      ...
      <table className="table md:w-full  ">
        ...
        <tbody className="item-center">
          {users.map((user, index) => (
            <tr key={index} className ={index % 2 === 0 ? 'text-orange-600' : 'text-gray-500'}>
              {/* <th className="hidden md:block">{index + 1}</th> */}
              <td className="md:flex md:items-center md:gap-2"><img src={user.photo} className="mask mask-squircle w-12 h-12" alt="user" /><span className="font-bold md:text-xl">{user.name}</span></td>
              <td className="font-semibold">{user.email}<br />{user.number}</td>
